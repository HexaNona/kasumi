...
export default class WebSocketSource extends MessageSource {
    ...
    private connectSocket() {
        if (this.url) {
            ...
            if (this.sessionId) {
                this.url +=
                    "&resume=1&sessionId=" + this.sessionId + "&sn=" + this.sn;
            }
            ...
            this.socket.on("error", function (error: Error) {
                ...
                if (
                    ...
                    botInstance.logger.warn(
                        "Fail to Connect to Kaiheila, retrying",
                        error
                    );
            ...
            this.socket.on("close", function (code, reason) {
                ...
                if (botInstance.helloTimeout)
                    botInstance.helloTimeout = clearTimeout(
                        botInstance.helloTimeout
                    );
                if (botInstance.heartbeatInterval)
                    botInstance.heartbeatInterval = clearInterval(
                        botInstance.heartbeatInterval
                    );
                if (botInstance.heartbeatTimeout)
                    botInstance.heartbeatTimeout = clearTimeout(
                        botInstance.heartbeatTimeout
                    );
                if (botInstance.stage === 3) {
                    botInstance.retry(
                        new Error(
                            "close before hello packet " + code + " " + reason
                        )
                    );
                }
        ...
        switch (this.stage) {
            ...
            case 1:
                if (this.retryTimes > 3) {
                    this.logger.warn(
                        "getGateWay Fail over three times, retrying",
                        error
                    );
                }
            ...
            case 5:
                ...
                if (this.retryTimes < 3) {
                    ...
                    this.logger.warn(
                        "heartbeat without reponse over three times",
                        error
                    );
                    try {
                        this.safeCloseSocket();
    ...
    private startHeartbeat() {
        if (this.heartbeatInterval) {
            clearInterval(this.heartbeatInterval);
            this.logger.warn(
                "Exist Heartbeat Interval , may happen something unexpected"
            );
        }
