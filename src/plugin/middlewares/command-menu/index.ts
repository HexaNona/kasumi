...
export class CommandMenu {
    static async middleware(
        ...
        if (
            (command instanceof Plugin &&
                session.args[0] == `(met)${session.client.me.userId}(met)` &&
                session.args.length == 1) ||
            (!(command instanceof Plugin) && command instanceof BaseMenu)
        ) {
            const card = new Card().addTitle("命令列表").addDivider();
