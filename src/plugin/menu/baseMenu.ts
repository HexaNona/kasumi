...
> extends BaseCommand<T> {
    ...
    addCommand(command: BaseMenu | BaseCommand<Kasumi<any>>) {
        ...
        if (!command.isInit)
            command.init(this.client, [...this.loggerSequence, command.name]);
    }
    addAlias(
        command: BaseMenu | BaseCommand<Kasumi<any>>,
        ...aliases: string[]
    ) {
        this._hierachyCacheUpToDate = false;
    ...
    fullHierachyCommands() {
        if (this._hierachyCacheUpToDate && this._fullHierachyCommands)
            return this._fullHierachyCommands;
